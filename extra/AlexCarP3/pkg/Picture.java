...
public class Picture implements Shape
{
   ...
   public Picture(double width, double height)
   {
	...
	public Picture(String source, double xx, double yy)
	{
		name = source;
		load(source);
		translate(xx,yy);
	...
	// public boolean touchingColorAt(int x, int y, Color color)
	// {
		...
		// {
			...
			// {
				// int rgb = image.getRGB(xx, yy) & 0xFFFFFF;
				// Color cc = new Color(rgb / 65536, (rgb / 256) % 256, rgb % 256);
				
				// if(cc == color && xx != x && yy != y)
				// {
					// return true;
				// }
				// else
	...
	public void alterPic(Color[][] pixels)
	{
		for(int y=0; y < this.getHeight(); y++)
		{
			for(int x=0; x < this.getWidth();x++)
			{
				this.setColorAt(x,y,pixels[x][y]);
			}
		}	
	}
	public void alterPic(Color[] pixels)
	{
		for(int y=0; y<this.getHeight(); y++)
		{
			for(int x=0; x<this.getWidth(); x++)
			{
				this.setColorAt(x,y,pixels[y* this.getWidth()+x]);
			}
		}	
	}
}
